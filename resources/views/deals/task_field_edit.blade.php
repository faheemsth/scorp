<div class="d-flex align-items-baseline p-1" style="background-color: #f6f6f6;border:1px solid rgb(199, 199, 199)">
    <div class="input-group" style="border-color: rgb(44, 128, 255)">
        @if($name == 'branch_id' || $name == 'organization_id' || $name == 'assigned_to' || $name == 'status' || $name == 'deal_id')
        ...
        @else
        <input type="text" name="{{$name}}" class="form form-control {{$name}} bg-transparent" value="{{$task->$name}}" style="border: 0px;box-shadow:none;border-raduis:0px;padding:4px !important;">
        <span class="input-group-text border-0 bg-transparent">
            <i class="ti ti-wand"></i>
        </span>
        @endif

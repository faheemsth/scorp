...
//    }
    ...
    public function DealSettingStore(Request $request){
        $validator = Validator::make(
            $request->all(), [
                               'deal_stage' => 'required|max:20'
                           ]
        );
        ...
        if(!$deal_stage){
            DB::table('settings')->insert([
                ...
                'value' => $request->deal_stage,
            ]);
        }else{
            DB::table('settings')
            ->where('id', '=', $deal_stage->id)
            ->update(['value' => $request->deal_stage]);
        }
